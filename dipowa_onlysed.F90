...
1170  continue
      ...
      DO 1421 i=1,kpie
         ...
         tredsy(i,k,2) = bolven(i)*bolay_clim(i,j,imonth) - tredsy(i,k,1) - tredsy(i,k,3) ! dz(kbo) - diff upper - diff lower
1421  CONTINUE
      ...
      DO 1422 i=1,kpie
         sedb1(i,k,iv)=0.
         IF(omask(i,j).GT.0.5)                                     &
     &      sedb1(i,k,iv)=ocetra_kbo_clim(i,j,iv_oc,imonth)*bolay_clim(i,j,imonth)*bolven(i)     ! tracer_concentration(kbo) * dz(kbo)
 1422 CONTINUE
