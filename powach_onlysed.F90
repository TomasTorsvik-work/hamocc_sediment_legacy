...
! Approximation for new solid sediment, as from sedimentation flux: solrat(i,1)
      ...
      DO 3 i=1,kpie
         IF(omask(i,j).GT.0.5) THEN
            ...
            sedb1(i,0)=bolay_clim(i,j,imonth)*(silsat-ocetra_kbo_clim(i,j,isilica,imonth)) &
     &                 *bolven(i)       
            solrat(i,1)=                                                &
...
! Approximation for new solid sediment, as from sedimentation flux: solrat(i,1)
      ...
      DO 13 i=1,kpie
         IF(omask(i,j).GT.0.5) THEN
            ...
            sedb1(i,0)=bolay_clim(i,j,imonth)*ocetra_kbo_clim(i,j,ioxygen,imonth)         &
     &                 *bolven(i)       
            solrat(i,1)=                                               &
...
! AND DIC GAIN. ITERATE 5 TIMES. THIS CHANGES PH (SEDHPL) OF SEDIMENT.
      ...
      DO 1 i=1,kpie
         IF(omask(i,j).GT.0.5) THEN
            ...
            ah1 = sedhpl(i,j,k)
            K1  = keqb_clim( 1,i,j,imonth)
            K2  = keqb_clim( 2,i,j,imonth)
            Kb  = keqb_clim( 3,i,j,imonth)
            Kw  = keqb_clim( 4,i,j,imonth)
            Ks1 = keqb_clim( 5,i,j,imonth)
            Kf  = keqb_clim( 6,i,j,imonth)
            Ksi = keqb_clim( 7,i,j,imonth)
            K1p = keqb_clim( 8,i,j,imonth)
            K2p = keqb_clim( 9,i,j,imonth)
            K3p = keqb_clim(10,i,j,imonth)
...
! Approximation for new solid sediment, as from degradation: solrat(i,k) 
      ...
      DO 22 i=1,kpie
         IF(omask(i,j).GT.0.5) THEN
            undsa=MAX(keqb_clim(11,i,j,imonth)/calcon-powcar(i,k),0.)
